...
final class Router
{
    ...
    public function run(): Response
    {
        ...
        if ($action === 'accueil') {
            ...
            $transport = new \Swift_SmtpTransport('smtp.sfr.fr', 465, 'SSL');
            $transport->setUsername($this->env['USER_MAIL']);
            $transport->setPassword($this->env['PASSWORD_MAIL']);
            $mailerService = new MailerService($transport, $this->session);

            $controller = new HomeController(new ArticleRepository($this->database), new UserRepository($this->database), $this->view);
            return $controller->contact($this->request, $mailerService);
        }
